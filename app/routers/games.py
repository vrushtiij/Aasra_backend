...
@router.get("/patients/{patient_id}/game-sessions")
def get_game_sessions(
    ...
    """Caregiver-facing game history for the selected patient."""
    assert_can_access_patient(patient_id, user, db)
    days = max(1, min(days, 365))
    since = datetime.now() - timedelta(days=days)
    rows = (
        db.query(models.GameSession, models.CognitiveGame)
        .join(
            models.CognitiveGame,
            models.CognitiveGame.game_id == models.GameSession.game_id,
        )
        .filter(
            models.GameSession.patient_id == patient_id,
            models.GameSession.start_time >= since,
        )
    ...
    return [
        {
            ...
            "accuracy": float(session.accuracy) if session.accuracy is not None else None,
            "difficulty": session.difficulty,
            "completion_status": session.completion_status,
        }
        for session, game in rows
    ]
